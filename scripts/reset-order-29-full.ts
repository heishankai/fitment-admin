...
/**
 * 重置订单29的所有验收状态并清空钱包（包括余额和冻结金额）
 * 使用方法：ts-node scripts/reset-order-29-full.ts
 */
async function resetOrder29Full() {
  // 创建数据库连接
  ...
  try {
    ...
    if (order.craftsman_user_id) {
      ...
      if (wallet) {
        ...
        console.log(`  原始冻结金额: ${oldFreezeMoney} -> 0`);
        console.log('✅ 钱包已清空（包括余额和冻结金额）');
      } else {
